...
| `internal/store/` | SQLite persistence (`modernc.org/sqlite`, pure Go — keeps the binary CGO-free and statically linked). Two tables: `requests` (one row per proxied call, cost fixed at write time) and `kills` (kill-switch state). Writes are serialised on a single connection, because SQLite has exactly one writer and a pool merely fights over the lock until `busy_timeout` expires — which silently drops usage records. `Aggregate` powers `GET /usage`; `Spend` powers the budget engine's re-sync. |
...
- **`requests`** — one row per proxied call: timestamp, agent/team/namespace,
  ...
  `unpriced_requests`. See [correctness.md](docs/correctness.md).
