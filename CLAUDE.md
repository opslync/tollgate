...
- Usage is parsed by a tee reader wrapped around the response body in `ModifyResponse`; one structured `slog` line per request when the body completes. Parse failures never break the proxy. Metering + the Recorder call run in a `defer` in `ServeHTTP`: `httputil.ReverseProxy` panics with `http.ErrAbortHandler` on a mid-stream client disconnect, and without the defer every cancelled stream went unrecorded (a budget bypass).
